        ...
        .then(collected => {
            ...
            if (reaction.emoji.name === '👍') {
                ...
                if(output[2]) {
                    ...
                    embed.setTitle(basicNestLanguage.search.statewide)
                    embed.setFooter(`${eachnest.sort().slice(0, 21).length} - ${eachnest.sort().slice(0, 30).length} ${basicNestLanguage.search.footer.a} ${eachnest.length} ${basicNestLanguage.search.footer.b} ${nestpropvalue}, ${serverstate}`)
                }
                ...
                setTimeout(function(){
                    if(eachnest.length > 30) {
                        ...
                        if(output[2]) {
                            ...
                            embed.setFooter(`${eachnest.sort().slice(0, 31).length} - ${eachnest.sort().slice(0, 40).length} ${basicNestLanguage.search.footer.a} ${eachnest.length} ${basicNestLanguage.search.footer.b} ${nestpropvalue}, ${serverstate}`)
                        }
                        bot.users.get(theuser).send(embed)
                    }
                }, 1500);
