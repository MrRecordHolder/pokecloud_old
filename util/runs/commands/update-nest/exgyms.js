exports.run = (bot, message) => {
    // set new data value
    bot.defaultNest.set(nestKey, nestData, 'exgyms')
    const exgyms = bot.defaultNest.get(nestKey, 'exgyms')
    ...
    var CONFIRM = new Discord.RichEmbed()
        ...
        if(nestData === "1") {
            CONFIRM.setTitle(`${nestName} ${success.response.updated.a} ${nestData} ${basicNestLanguage.exgyms}`)
        } else {
            CONFIRM.setTitle(`${nestName} ${success.response.updated.a} ${nestData} ${basicNestLanguage.exgyms}`)
        }
        CONFIRM.setThumbnail("https://github.com/MrRecordHolder/pokecloud/blob/master/images/emojis/exgym.png?raw=true")
    ...
    bot.channels.get(nestchannel).fetchMessage(nestmsgid).then(editEmbed => {
        ...
        embed = new RichEmbed (editEmbed.embeds[0])
            ...
            embed.setFooter(basicNestLanguage.exgyms + " " + success.response.updatedby.a + " " + username, userimage)
            embed.setTimestamp()
