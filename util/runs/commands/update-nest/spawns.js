exports.run = (bot, message) => {
    // set new data value
    bot.defaultNest.set(nestKey, nestData, 'spawns')
    ...
    var CONFIRM = new Discord.RichEmbed()
        ...
        if(nestData === "1") {
            CONFIRM.setTitle(`${nestName} ${success.response.updated.a} ${nestData} ${basicNestLanguage.spawns}`)
        } else {
            CONFIRM.setTitle(`${nestName} ${success.response.updated.a} ${nestData} ${basicNestLanguage.spawns}`)
        }
        CONFIRM.setThumbnail("https://github.com/MrRecordHolder/pokecloud/blob/master/images/emojis/spawn.png?raw=true")
    ...
    bot.channels.get(nestchannel).fetchMessage(nestmsgid).then(editEmbed => {
        ...
        embed = new RichEmbed (editEmbed.embeds[0])
            ...
            embed.setFooter(basicNestLanguage.spawns + " " + success.response.updatedby.a + " " + username, userimage)
            embed.setTimestamp()
